...
import { updateProduct } from "@/store/features/shopsSlice"
import IProduct from "@/interfaces/Product.interface"
import { Edit3, Package, Image as ImageIcon, FileText, DollarSign, Hash, Loader2, AlertCircle } from "lucide-react"
...
const ProductEditForm: React.FC<ProductEditFormProps> = ({ product, shopId }) => {
    const [isOpen, setIsOpen] = useState(false)
    const [isSubmitting, setIsSubmitting] = useState(false)
    const [errors, setErrors] = useState<Record<string, string>>({})
    const dispatch = useAppDispatch()

    // Inicializar formulario con datos del producto actual
        ...
        <Dialog open={isOpen} onOpenChange={(open) => {
            ...
            <DialogTrigger asChild>
                <Button
                    size="sm"
                    variant="ghost"
                    className="h-10 w-10 p-0 text-gray-400 hover:text-blue-500 hover:bg-blue-50 opacity-0 group-hover:opacity-100 transition-all duration-200"
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                                Modificar <span className="font-medium text-blue-600">{product.name}</span>
                            </p>
                        </div>
                    </div>
                </DialogHeader>
                ...
                <form className="space-y-4 mt-6" onSubmit={handleSubmit}>
                    ...
                    <div className="space-y-2">
                        ...
                        {form.image && !errors.image && (
                            <p className="text-xs text-blue-600">✓ URL válida</p>
                        )}
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Textarea 
                            ...
                            onChange={handleChange}
                            className={`resize-none min-h-[60px] transition-colors ${errors.description ? 'border-red-300 focus:border-red-500' : 'focus:border-blue-500'}`}
                            disabled={isSubmitting}
                        />
                        {errors.description && (
                            <p className="text-xs text-red-600 flex items-center gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input 
                                ...
                                value={form.price} 
                                onChange={handleChange}
                                className={`transition-colors ${errors.price ? 'border-red-300 focus:border-red-500' : 'focus:border-blue-500'}`}
                                disabled={isSubmitting}
                            />
                            {previewPrice && !errors.price && (
                                <p className="text-xs text-blue-600">{previewPrice}</p>
                            )}
                            {errors.price && (
                                <p className="text-xs text-red-600 flex items-center gap-1">
                                    <AlertCircle className="w-3 h-3" />
                        ...
                        <div className="space-y-2">
                            ...
                            <Input 
                                ...
                                value={form.quantity} 
                                onChange={handleChange}
                                className={`transition-colors ${errors.quantity ? 'border-red-300 focus:border-red-500' : 'focus:border-blue-500'}`}
                                disabled={isSubmitting}
                            />
                            {errors.quantity && (
                                <p className="text-xs text-red-600 flex items-center gap-1">
